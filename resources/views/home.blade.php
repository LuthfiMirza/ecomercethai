...
<section class="container mx-auto px-6 py-10">
  ...
  <div class="flex flex-wrap justify-center gap-6">
    @foreach(['GPU','CPU','SSD','Monitor'] as $cat)
      <a href="{{ route('catalog', ['category' => $cat]) }}" class="group w-[180px]">
        <div class="rounded-2xl border border-neutral-200 bg-white p-4 shadow-soft transition hover:-translate-y-1 hover:shadow-lg">
          <img loading="lazy" src="https://source.unsplash.com/160x160/?{{ urlencode($cat) }}" alt="{{ $cat }}" class="mx-auto w-[96px] h-[96px] object-cover rounded-xl"/>
          <span class="mt-3 block text-center text-sm font-semibold text-neutral-800">{{ $cat }}</span>
        </div>
      </a>
    @endforeach
  </div>
...
<div class="container mx-auto px-6">
    <h2 class="text-2xl font-bold text-gray-900 mb-8 text-center">{{ __('Recommended Products') }}</h2>
      ...
      <article class="group rounded-2xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 shadow-soft hover:shadow-elevated transition overflow-hidden">
        <a href="{{ route('catalog') }}" class="block">
          ...
          <div class="p-4 space-y-3">
            ...
            <button data-cart-add data-product-id="{{ $product->id }}" data-name="{{ $product->name }}" data-price="{{ $price }}" data-image="{{ $image }}" class="w-full inline-flex items-center justify-center gap-2 rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-800 py-2 text-sm font-medium text-neutral-700 dark:text-neutral-200 hover:bg-neutral-50 dark:hover:bg-neutral-800">
