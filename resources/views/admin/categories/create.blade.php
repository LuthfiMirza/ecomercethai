...
@section('header', __('admin.categories.create_title'))

@section('content')
<div class="bg-white dark:bg-slate-800 shadow-md rounded-lg p-6">
    <h2 class="text-2xl font-semibold text-slate-800 dark:text-slate-100 mb-6">{{ __('admin.categories.form_heading') }}</h2>

    <form action="{{ localized_route('admin.categories.store') }}" method="POST">
        @csrf
        <div class="mb-4">
            <label for="name" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{{ __('admin.categories.name_label') }}</label>
        ...
        <div class="mb-6">
            <label for="description" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{{ __('admin.categories.description_label') }}</label>
        ...
        <div class="flex items-center justify-end space-x-4">
            <a href="{{ localized_route('admin.categories.index') }}" class="px-4 py-2 bg-slate-200 text-slate-800 rounded-lg hover:bg-slate-300 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600">{{ __('admin.categories.cancel') }}</a>
