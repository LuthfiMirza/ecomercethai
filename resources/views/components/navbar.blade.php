<header x-data="Object.assign(navbar(), { openMega:false, lastFocus:null, headerH:72, updateHeader(){ const r=this.$refs.hdr?.getBoundingClientRect?.(); this.headerH=r?Math.round(r.height):72; } })"
        ...
        x-ref="hdr"
        class="sticky top-0 z-[90] bg-white/90 dark:bg-neutral-900/80 backdrop-blur border-b border-neutral-200 dark:border-neutral-800"
        role="banner">
  <!-- Topbar -->
  <div class="hidden md:block bg-neutral-50 dark:bg-neutral-900/90 border-b border-neutral-200/70 dark:border-neutral-800">
    <div class="container mx-auto px-4 text-xs text-neutral-600 dark:text-neutral-300 flex items-center justify-between h-8">
      <div class="flex items-center gap-4">
        <a href="#" class="hover:text-accent-600 focus:outline-none focus:ring-2 focus:ring-accent-500 rounded">About</a>
        <a href="#" class="hover:text-accent-600 focus:outline-none focus:ring-2 focus:ring-accent-500 rounded">FAQs</a>
        <a href="#" class="hover:text-accent-600 focus:outline-none focus:ring-2 focus:ring-accent-500 rounded">Contact</a>
        <a href="#" class="hover:text-accent-600 focus:outline-none focus:ring-2 focus:ring-accent-500 rounded">Track Order</a>
      </div>
      <div class="flex items-center gap-4">
        <span class="text-neutral-500">Hotline: <span class="font-medium text-neutral-800 dark:text-neutral-100">+66 1234 5678</span></span>
        <x-language-switcher />
        <button @click="toggleTheme" class="p-1.5 rounded hover:bg-neutral-100 dark:hover:bg-neutral-800" aria-label="Toggle theme">
  ...
  <div class="container mx-auto px-4">
    <div class="flex h-16 items-center gap-3 justify-between">
      ...
      <!-- Center Search -->
      <div class="hidden md:block w-full max-w-xl mx-2 md:mx-6" x-data="{open:false}" @click.outside="open=false">
        <form action="{{ url('/search') }}" method="get" role="search" class="relative" @submit="open=false">
          <label for="q" class="sr-only">Search products</label>
          <input id="q" name="q" type="search" :placeholder="t('search_placeholder')" @focus="open=true" @keydown.escape.window="open=false" class="w-full rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 pl-11 pr-24 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-500 text-neutral-800 dark:text-neutral-100"/>
        ...
        <!-- Suggestions -->
        <div x-cloak x-show="open" class="absolute mt-2 w-full bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-xl shadow-elevated p-2 z-50">
          ...
          <ul class="text-sm">
            @foreach(['RTX 4070 Ti','Gaming laptop','NVMe SSD 1TB','4K Monitor'] as $s)
              <li><a class="block px-3 py-2 hover:bg-neutral-50 dark:hover:bg-neutral-800 rounded" href="{{ url('/catalog?q='.urlencode($s)) }}">{{ $s }}</a></li>
            @endforeach
          </ul>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3">
        <!-- Dark mode toggle -->
        <button @click="toggleTheme" class="p-2 rounded-full hover:bg-neutral-100 dark:hover:bg-neutral-800" aria-label="Toggle theme">
          <i x-show="isDark" class="fa-solid fa-moon text-neutral-700 dark:text-neutral-200"></i>
          <i x-show="!isDark" class="fa-solid fa-sun text-yellow-500"></i>
        </button>
        ...
        <!-- Wishlist -->
        <a href="#" class="relative inline-flex items-center p-2 rounded-full hover:bg-neutral-100 dark:hover:bg-neutral-800" aria-label="Wishlist">
        ...
        <button class="md:hidden p-2 rounded-full hover:bg-neutral-100 dark:hover:bg-neutral-800" @click="openSearch = true" aria-label="Open search">
          <i class="fa-solid fa-magnifying-glass"></i>
        </button>

        <!-- Mobile categories button -->
        <button class="md:hidden p-2 rounded-md border border-neutral-200 dark:border-neutral-700" @click="lastFocus=$event.currentTarget; openMega=true" aria-label="Open categories">
    ...
    <nav class="hidden md:flex items-center gap-6 h-12 text-sm" aria-label="Primary">
      ...
      <button
        @click="lastFocus=$event.currentTarget; openMega=true"
  ...
  <div x-cloak x-show="openMega" x-transition.opacity class="fixed inset-0 z-[80]" aria-hidden="true">
    <!-- overlay -->
      <div class="absolute inset-0 bg-black/40" @click="openMega=false; lastFocus && lastFocus.focus()"></div>
      ...
      <div class="container mx-auto px-4 py-6">
        <div class="flex items-start justify-between">
          ...
          <button class="rounded-xl px-3 py-2 hover:bg-neutral-100 dark:hover:bg-neutral-800 focus:outline-none focus:ring-2 focus:ring-primary-500"
                  @click="openMega=false; lastFocus && lastFocus.focus()" aria-label="Close menu">✕</button>
        </div>
          ...
          @foreach($cats as $cat)
            <section class="rounded-2xl border border-neutral-200 dark:border-neutral-800 p-4 hover:shadow-soft transition">
              ...
              <ul class="space-y-1 text-sm text-neutral-600 dark:text-neutral-300">
                @foreach($cat['links'] as [$label, $slug])
  ...
  <div x-cloak x-show="open" x-transition.opacity class="fixed inset-0 z-50" aria-hidden="true">
    ...
    <aside class="absolute right-0 top-0 h-full w-[88vw] max-w-sm bg-white dark:bg-neutral-900 shadow-elevated p-4 overflow-y-auto" role="dialog" aria-label="Mobile menu">
      <div class="flex items-center justify-between mb-4">
        ...
        <button @click="open=false" class="p-2 rounded hover:bg-neutral-100 dark:hover:bg-neutral-800" aria-label="Close menu"><i class="fa-solid fa-xmark"></i></button>
      </div>
  ...
  <div x-cloak x-show="openSearch" x-transition.opacity class="fixed inset-0 z-50" aria-hidden="true">
    <div class="absolute inset-0 bg-black/40" @click="openSearch=false"></div>
    <div class="absolute inset-x-4 top-16 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-xl shadow-elevated p-3">
      <form action="{{ url('/search') }}" method="get" role="search" class="flex items-center gap-2">
        <label for="mq" class="sr-only">Search products</label>
        <i class="fa-solid fa-magnifying-glass text-neutral-400"></i>
        <input id="mq" name="q" type="search" placeholder="Cari produk..." class="flex-1 bg-transparent outline-none text-sm text-neutral-800 dark:text-neutral-100"/>
