  ...
  <section class="soft-card p-6 md:p-8 space-y-6">
    <header class="space-y-2">
      ...
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 class="text-3xl font-semibold text-neutral-900 dark:text-neutral-100">{{ __('Pembayaran via Xendit') }}</h1>
          <p class="text-sm text-neutral-500 dark:text-neutral-400">{{ __('Bayar menggunakan virtual account, e-wallet, atau gerai minimarket melalui Xendit.') }}</p>
        </div>
    ...
    <div class="soft-card p-5 bg-white/80 dark:bg-neutral-900/60 space-y-4">
      <h2 class="text-sm font-semibold text-neutral-900 dark:text-neutral-100">{{ __('Status Pembayaran') }}</h2>
      <p class="text-sm text-neutral-600 dark:text-neutral-300">{{ __('Invoice Xendit akan dibuat ketika integrasi aktif. Saat ini Anda dapat menggunakan ringkasan di bawah sebagai panduan.') }}</p>
      <div class="grid gap-3 md:grid-cols-2 text-sm text-neutral-600 dark:text-neutral-300">
        <div>
          <div class="text-xs uppercase tracking-wide text-neutral-400">{{ __('Total Tagihan') }}</div>
          <div class="text-lg font-semibold text-neutral-900 dark:text-neutral-100">{{ format_price($order->total_amount) }}</div>
        </div>
        <div>
          <div class="text-xs uppercase tracking-wide text-neutral-400">{{ __('Status') }}</div>
          <div class="text-sm font-medium text-neutral-700 dark:text-neutral-200">{{ ucfirst($order->payment_status) }}</div>
        </div>
      </div>
      <x-button type="button" class="inline-flex items-center gap-2">
        <i class="fa-solid fa-file-invoice-dollar text-xs"></i>
        <span>{{ __('Generate Invoice Xendit') }}</span>
      </x-button>
      <x-alert type="info" class="text-xs">{{ __('Hubungkan API key Xendit Anda untuk mengaktifkan pembuatan invoice otomatis.') }}</x-alert>
    </div>
